<template>
  <div>
    <div
      v-if="$store.state.errorMessageDisplay"
      class="m-auto bg-danger text-light px-3 py-3 text-center botder border-dark border-5 mb-3"
      style="border:3px solid black"
    >
      {{ $store.state.errorMessageText }}
    </div>
...
export default {
  ...
  methods: {
    getAllFavorites() {
      axios
        .get('http://localhost/new-weather-api-laravel/public/get-all-favorites')
        .then((response) => {
    ...
    getWeather() {
      ...
      axios
        .get(URL)
        .then((response) => {
          this.$store.commit('updateMainWeatherDisplay', response.data.current.temp_c + 'C')
